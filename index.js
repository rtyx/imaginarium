...
app.get('/delete/:id',function(req,res){
  db.query('DELETE FROM images WHERE id=$1',[req.params.id],function(err,data){
    console.log(data);
    res.json({images:data});
  });
});
...
app.post('/update/:id',function(req,res){
  var form= req.body;
  var param = [form.id,form.user_name, form.title, form.description];
  var query= "UPDATE images SET user_name = $1, title = $2, description = $3 WHERE id = $4";

  if(form.user_name || form.title || form.description != ""){
    db.query(query,param,function(err,results){
      if(err){
        console.log(err);
      } else {
